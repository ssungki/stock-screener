...
set -e
cmd="${1:?command required (post_daily_report|post_weekly_report|post_total_report|post_breakout_scan|backfill|paper_open|paper_close)}"
case "$cmd" in
    post_daily_report|post_weekly_report|post_total_report|post_breakout_scan|backfill|paper_open|paper_close) ;;
